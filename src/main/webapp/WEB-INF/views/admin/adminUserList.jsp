			...
			<form action="/admin/userList" method="get" class="row g-3">
				<div class="col-auto">
					<select name="searchType" class="form-select">
						...
						<option value="mobile"
							${param.searchType=='mobile' ? 'selected' : '' }>전화번호</option>
					</select>
				</div>
				<div class="col-auto">
				...
				<div class="col-auto">
					<button type="button" class="btn btn-secondary"
						onclick="resetSearch()">초기화</button>
				</div>
			</form>
		</div>
							...
							<c:forEach items="${userList}" var="user">
								<tr>
									...
									<td>
										...
										</button> <c:if test="${user.blockDeadline == null}">
											<button class="btn btn-danger btn-sm"
												onclick="blockUser(${user.uid})">
												<i class="fas fa-ban"></i> 정지
											</button>
										</c:if> <c:if test="${user.blockDeadline != null}">
