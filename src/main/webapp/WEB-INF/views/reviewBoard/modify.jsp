...
<head>
  ...
  function toggleOtherTypeInput() {
	  ...
	  // 기타 입력값을 localStorage에 저장해서 submit 시 같이 보낼 수 있게
...
<div class="container mt-5">
  ...
  <form action="${pageContext.request.contextPath}/reviewBoard/modify" method="post" onsubmit="return validateForm()">
    ...
    <input type="hidden" name="companyName" id="companyNameInput" value="${writeBoardDTO.companyName}" />
		...
		<c:set var="typeList" value="${fn:split('FACE_TO_FACE,VIDEO,PHONE,OTHER', ',')}" />
		<c:forEach var="type" items="${typeList}">
		  <div class="form-check form-check-inline">
		    <input class="form-check-input" type="radio" name="reviewType" value="${type}"
		      <c:if test="${type == writeBoardDTO.reviewType}">checked</c:if>>
		    <label class="form-check-label">${type}</label>
		  </div>
		</c:forEach>

  <!-- 기타 선택 시 설명 입력 -->
  <div id="otherTypeWrapper" class="mt-2" style="display: none;">
    <input type="text" class="form-control" id="otherTypeText" placeholder="기타 면접 유형을 입력해주세요.">
  </div>
</div>
    ...
    <div class="mb-3">
      ...
      <div class="form-check mb-2">
        <input class="form-check-input" type="checkbox" id="resultConsent" checked onchange="document.getElementById('reviewResultSelect').disabled = !this.checked">
